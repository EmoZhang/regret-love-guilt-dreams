...
sizes = [len(no_record_list), theme_count, search_count]
...
ax.set_title('Non-Hit Tag Distribution\n({} songs out of {})'.format(len(no_record_list), sum(sizes)))
...
autolabel(rects)
plt.savefig('pics/non_hit_tag_distribution.png')
...
ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
ax1.set_title(
    'Non-Hit Tag Clustering\n({} songs out of {})'.format(sum(sizes), len(no_record_list)))
plt.savefig('pics/non_hit_tag_clustering.png')
